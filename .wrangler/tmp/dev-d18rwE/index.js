...
// lib/auth.js
var ADMIN_EMAILS = ["anymousxe.info@gmail.com"];
var ADMIN_USERNAMES = ["anymousxe"];
...
__name(getSupabaseAnon, "getSupabaseAnon");
...
async function authenticateApiKey(key, env) {
  ...
  const { data: keyData, error } = await sb.from("user_api_keys").select("id, user_id, active").eq("key", key).single();
  if (error || !keyData || !keyData.active) {
    throw { status: 401, message: "invalid api key" };
  }
  sb.from("user_api_keys").update({ last_used_at: (/* @__PURE__ */ new Date()).toISOString(), request_count: keyData.request_count + 1 }).eq("id", keyData.id).then(() => {
  });
  const { data: credits } = await sb.from("user_credits").select("plan, balance, plan_expires_at").eq("user_id", keyData.user_id).single();
  ...
  if (credits?.plan_expires_at && new Date(credits.plan_expires_at) < /* @__PURE__ */ new Date() && !isAdmin) {
    plan = "free";
  }
  return {
...
async function authenticateSupabase(token, env) {
  ...
  const isAdmin = ADMIN_EMAILS.includes(email) || ADMIN_USERNAMES.includes(username);
  const sb = getSupabase(env);
  const { data: credits } = await sb.from("user_credits").select("plan, balance, plan_expires_at").eq("user_id", user.id).single();
  let plan = credits?.plan || "free";
  if (isAdmin) plan = "admin";
  if (credits?.plan_expires_at && new Date(credits.plan_expires_at) < /* @__PURE__ */ new Date() && !isAdmin) {
    plan = "free";
...
__name(authenticateSupabase, "authenticateSupabase");
...
var MODELS2 = {
  ...
  "gpt-5.4": {
    ...
    tier: "free",
    type: "chat",
    description: "Latest reasoning model. Handles complex multi-step problems with ease.",
  ...
  "gemini-3.1-pro-preview": {
    ...
    tier: "free",
    type: "chat",
    description: "Multimodal flagship with a massive context window. Preview access.",
  ...
  "gemini-3-flash": {
    ...
    tier: "free",
    type: "chat",
    description: "Fast and efficient Gemini model for everyday tasks.",
  ...
  "kimi-k2.5": {
    ...
    tier: "free",
    type: "chat",
    description: "Strong at long-context reasoning and code generation.",
  ...
  "deepseek-v3.2": {
    ...
    tier: "free",
    type: "chat",
    description: "Strong coder and reasoning model. Excellent cost-to-performance ratio.",
  ...
  "qwen3-coder": {
    ...
    tier: "free",
    type: "chat",
    description: "Exceptional coding performance with very high rate limits.",
  ...
  "healer-alpha": {
    id: "healer-alpha",
    name: "Healer Alpha",
    provider: "AnymousxeAPI",
    tier: "free",
    type: "chat",
    description: "Specialized model for helpful and empathetic conversations.",
  ...
  "hunter-alpha": {
    ...
    tier: "free",
    type: "chat",
    description: "Specialized model for research and information gathering.",
...
function getAllowedModels(plan) {
  const tierRank = { free: 0, plus: 1, pro: 2, admin: 99 };
  const rank = tierRank[plan] ?? 0;
  return Object.values(MODELS2).filter((m) => {
    if (m.tier === "internal") return false;
    return (tierRank[m.tier] ?? 0) <= rank;
  });
}
__name(getAllowedModels, "getAllowedModels");
function getAllowedModelIds(plan) {
  return getAllowedModels(plan).map((m) => m.id);
}
__name(getAllowedModelIds, "getAllowedModelIds");
function resolveModel(modelId) {
  const model = MODELS2[modelId];
  return model ? model.literouter_id : modelId;
}
__name(resolveModel, "resolveModel");
function isValidModel(modelId) {
  return modelId in MODELS2 && MODELS2[modelId].tier !== "internal";
}
__name(isValidModel, "isValidModel");

// lib/proxy.js
var BASE_URL = "https://api.literouter.com/v1";
var IMAGE_BASE_URL = "https://image.literouter.com";
function getBackendKeys(env) {
  const keys = [];
  for (let i = 1; i <= 15; i++) {
...
__name(getBackendKeys, "getBackendKeys");
async function proxyRequest(body, env) {
...
async function proxyImageRequest(body, env) {
  const keys = getBackendKeys(env);
  if (keys.length === 0) {
    return {
      status: 500,
...
__name(proxyImageRequest, "proxyImageRequest");

// lib/ratelimit.js
var windowMs = 60 * 1e3;
var store = /* @__PURE__ */ new Map();
function rateLimit(apiKey, maxRequests = 30) {
  const now = Date.now();
  ...
  if (timestamps.length >= maxRequests) {
    return {
      allowed: false,
      remaining: 0,
      resetIn: Math.ceil((timestamps[0] + windowMs - now) / 1e3)
    };
  }
  timestamps.push(now);
  return {
    ...
    resetIn: Math.ceil(windowMs / 1e3)
  };
}
...
function calculateCost(modelId, inputTokens, outputTokens) {
  ...
  const inputCost = inputTokens / 1e6 * (model.pricing.input || 0);
  const outputCost = outputTokens / 1e6 * (model.pricing.output || 0);
...
__name(isFreeModel, "isFreeModel");
function canAfford(modelId, balance) {
  if (isFreeModel(modelId)) return true;
  return balance > 0;
}
__name(canAfford, "canAfford");
async function deductCredits(env, userId, amount, modelId, inputTokens, outputTokens, apiKeyId) {
  if (amount <= 0) return;
...
async function getBalance(env, userId) {
  const sb = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data } = await sb.from("user_credits").select("balance, plan, plan_expires_at, monthly_allowance, last_refill_at").eq("user_id", userId).single();
  return data;
}
__name(getBalance, "getBalance");
async function getTransactions(env, userId, limit = 50) {
  const sb = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data } = await sb.from("credit_transactions").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(limit);
  return data || [];
}
__name(getTransactions, "getTransactions");
async function getUsageStats(env, userId, days = 30) {
  const sb = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1e3).toISOString();
  const { data } = await sb.from("credit_transactions").select("model, amount, tokens_in, tokens_out, created_at").eq("user_id", userId).eq("type", "usage").gte("created_at", since).order("created_at", { ascending: false });
  return data || [];
}
__name(getUsageStats, "getUsageStats");

// src/routes/completions.js
async function handleCompletions(request, env, ctx) {
  maybeCleanup();
  let user;
  try {
  ...
  const hasImages = messages.some(
    (m) => Array.isArray(m.content) && m.content.some((c) => c.type === "image_url" || c.type === "image")
  );
  ...
  try {
    ...
    if (isImageModel) {
      ...
      const promptContent = typeof lastUserMessage.content === "string" ? lastUserMessage.content : JSON.stringify(lastUserMessage.content);
      const imageBody = {
...
async function handleModels() {
  const models = Object.values(MODELS2).filter((m) => m.tier !== "internal").map((m) => ({
    id: m.id,
    object: "model",
    created: 17e8,
    owned_by: "anymousxe",
...
async function handleSendCode(request, env) {
  ...
  try {
    ...
    const { error: dbError } = await supabase.from("otp_codes").upsert({ email, code, expires_at: expiresAt2, attempts: 0 }, { onConflict: "email" });
...
// src/routes/auth-verify-code.js
async function handleVerifyCode(request, env) {
  ...
  try {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
    const { data: record, error } = await supabase.from("otp_codes").select("*").eq("email", email).single();
    if (error || !record) {
      return Response.json({ error: "No code found for this email" }, { status: 400 });
    }
    if (/* @__PURE__ */ new Date() > new Date(record.expires_at)) {
...
// src/routes/keys.js
async function handleKeys(request, env, path, method) {
  let user;
  try {
    user = await authenticate(request, env);
  } catch (err) {
  ...
  if (method === "GET" && path === "/v1/keys") {
    const { data, error } = await sb.from("user_api_keys").select("id, key, label, active, created_at, last_used_at, request_count").eq("user_id", user.userId).order("created_at", { ascending: false });
    if (error) {
  ...
  if (method === "DELETE" && path.startsWith("/v1/keys/")) {
    ...
    const { error } = await sb.from("user_api_keys").update({ active: false }).eq("id", keyId).eq("user_id", user.userId);
    if (error) {
...
async function handleCredits(request, env, path, method) {
  if (method !== "GET") {
...
__name(handleCredits, "handleCredits");
...
async function handleAdmin(request, env, path, method) {
  ...
  if (path === "/v1/admin/add-credits" && method === "POST") {
    ...
    const { data: existing } = await sb.from("user_credits").select("balance").eq("user_id", targetUser.id).single();
  ...
  if (path === "/v1/admin/users" && method === "GET") {
    const { data: credits } = await sb.from("user_credits").select("user_id, balance, plan, plan_expires_at, monthly_allowance");
    const { data: userData } = await sb.auth.admin.listUsers();
    const userMap = {};
    if (userData?.users) {
  ...
  if (path === "/v1/admin/set-plan" && method === "POST") {
    ...
    const expiresAt2 = plan !== "free" ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString() : null;
    await sb.from("user_credits").upsert({
      user_id: targetUser.id,
      plan,
      plan_expires_at: expiresAt2
    }, { onConflict: "user_id" });
    return Response.json({ success: true, email, plan, plan_expires_at: expiresAt2 });
  }
  return Response.json({ error: { message: "not found" } }, { status: 404 });
}
...
async function handleImageGen(request, env, ctx) {
  let user;
  try {
    user = await authenticate(request, env);
  } catch (err) {
...
async function handleMoonpayWebhook(request, env) {
  ...
  const sb = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data: existing } = await sb.from("moonpay_transactions").select("id").eq("moonpay_id", data.id).single();
  if (existing) {
  ...
  const planType = data.metadata?.plan || null;
  ...
  if (type === "transaction_completed" || data.status === "completed") {
    if (planType && ["plus", "pro"].includes(planType)) {
      ...
      await sb.from("user_credits").upsert({
        ...
        balance: allowance,
        last_refill_at: (/* @__PURE__ */ new Date()).toISOString()
      }, { onConflict: "user_id" });
      await sb.from("credit_transactions").insert([{
        user_id: userId,
        amount: allowance,
      ...
      const { data: credits } = await sb.from("user_credits").select("balance").eq("user_id", userId).single();
...
// .wrangler/tmp/bundle-Pj3Dcs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Pj3Dcs/middleware-loader.entry.ts
