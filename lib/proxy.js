...
export async function proxyRequest(body, env) {
    ...
    let lastError = 'No backend keys were successful.';
    for (let i = 0; i < keys.length; i++) {
        try {
            ...
            // Only retry on retryable errors (rate limit, auth, server errors)
            if (response.status === 429 || response.status === 401 || response.status === 403 || response.status >= 500) {
                const text = await response.text().catch(() => '');
                lastError = `Key ${i} failed (${response.status}): ${text.substring(0, 100)}`;
                console.error(`[proxyRequest] ${lastError}`);
                continue;
            }
            ...
            lastError = `Key ${i} threw error: ${err.message}`;
            console.error(`[proxyRequest] ${lastError}`);
            continue;
        }
    }
    ...
    return {
        ...
        body: {
            error: {
                message: `All keys failed. Please try again later.`,
                type: 'server_error',
