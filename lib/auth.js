...
async function authenticateApiKey(key, env) {
    console.log('[auth] authenticating key:', key);
    const sb = getSupabase(env);

    const { data: keyData, error } = await sb
        .from('user_api_keys')
        .select('id, user_id, active, request_count, rate_limit')
        .eq('key', key)
        .single();

    if (error || !keyData || !keyData.active) {
        console.error('[auth] key lookup failed:', error?.message || 'not found/inactive');
        throw { status: 401, message: 'invalid api key' };
    }

    console.log('[auth] key valid for user:', keyData.user_id);
    ...
    sb.from('user_api_keys')
        ...
        .eq('id', keyData.id)
        .then(() => console.log('[auth] usage updated'))
        .catch(e => console.error('[auth] usage update failed:', e.message));
    ...
    if (creditError) console.warn('[auth] credits lookup error:', creditError.message);

    // Get user email for admin check
    console.log('[auth] fetching user data from auth.admin...');
    const { data: userData, error: userError } = await sb.auth.admin.getUserById(keyData.user_id);
    if (userError) console.error('[auth] getUserById fail:', userError.message);
    ...
    console.log(`[auth] user=${email} plan=${plan} isAdmin=${isAdmin}`);
    ...
    return {
        ...
        rateLimit: keyData.rate_limit ?? 20,
    };
}
